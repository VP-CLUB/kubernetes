...
function init {
  ...
  SECRET= eval "${KUBECTL} get secret --namespace=kube-system | grep aliregistrykey | cat"
  if [ ! "$NAMESPACE" ]; then
  ...
  if [ ! "$SECRET" ]; then
   ./create-alisecret.sh
    echo "The secret 'aliregistrykey' is successfully created."
  else
    echo "The secret 'aliregistrykey' is already there. Skipping."
  fi
  echo
}
